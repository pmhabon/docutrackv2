...
<div class="container" style="max-width:600px">
    <div class="card">
        <div class="modal-header" style="background:linear-gradient(135deg,#800000,#a00000);color:#fff">
            <h5 class="modal-title"><i class="fas fa-graduation-cap"></i> Edit: {{ $program->name }}</h5>
        </div>
        <div class="card-body">
            @if ($errors->any())
            ...
            <form action="{{ route('programs.update', $program) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <textarea name="description" id="description" rows="4" class="form-control @error('description') is-invalid @enderror">{{ old('description', $program->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
