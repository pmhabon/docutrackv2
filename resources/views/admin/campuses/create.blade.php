...
@section('page-title')
    <i class="fas fa-plus-circle"></i> Create New Campus
@endsection
...
<div class="container" style="max-width:600px">
    <div class="card">
        <div class="modal-header" style="background:linear-gradient(135deg,#800000,#a00000);color:#fff">
            <h5 class="modal-title"><i class="fas fa-map-marker"></i> New Campus</h5>
        </div>
        <div class="card-body">
            @if ($errors->any())
            ...
            <form action="{{ route('campuses.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" 
                           placeholder="e.g., Main Campus" value="{{ old('name') }}" required>
                    @error('name')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="location" id="location" class="form-control @error('location') is-invalid @enderror" 
                           placeholder="e.g., Laoag City" value="{{ old('location') }}">
                    @error('location')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="description" id="description" rows="4" class="form-control @error('description') is-invalid @enderror" 
                              placeholder="Campus details...">{{ old('description') }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
